...
public class ResImpl extends BaseImpl<ResModel> implements ResDao {
	...
	public List<ResModel> getAllByEmpUuid(Long uuid) {
		...
		// 员工->角色->资源
		/*
		 * 员工找角色 员工1 角色1,2 员工2 角色3,4 角色5,6,7 员工3 from EmpModel em join em.roles
		 * //获取所有具有角色的员工信息 from RoleModel rm join rm.reses //获取所有具有资源的角色信息 from
		 * EmpModel em join em.roles rm join rm.reses res //获取具有资源的角色的员工信息
		 * 
		 * //默认不写select查询的是所有被关联的信息总和 select em,rm,res from EmpModel em join
		 * em.roles rm join rm.reses res //仅仅需要资源信息 select res from EmpModel em
		 * join em.roles rm join rm.reses res
		 */
