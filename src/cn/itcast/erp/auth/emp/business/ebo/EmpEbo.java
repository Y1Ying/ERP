...
public class EmpEbo implements EmpEbi {
	...
	public void update(EmpModel em) {
		...
		/*
		 * //做修改功能外界传递了一个em对象，有些东西不让修改 //不允许修改，将你修改的值恢复 EmpModel temp =
		 * empDao.get(em.getUuid());
		 * 
		 * em.setUserName(temp.getUserName());
		 * em.setBirthday(temp.getBirthday()); empDao.update(em);
		 */
	}
	...
	public List<EmpModel> getAll(BaseQueryModel qm, Integer pageNum,
			Integer pageCount) {
